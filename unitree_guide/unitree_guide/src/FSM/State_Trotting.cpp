...
State_Trotting::State_Trotting(CtrlComponents *ctrlComp)
    ...
    _q_contact=ctrlComp->_q_contact;
    _gaitHeight = 0.08;
    ...
    _Kpp = Vec3(20, 20, 1000).asDiagonal();
    _Kdp = Vec3(20, 20, 100).asDiagonal();
    _kpw = 400;
...
void State_Trotting::run(){
    ...
    if(checkStepOrNot()){
        _ctrlComp->setStartWave();    //到时候做地面适应应该可以塞进这个里面
...
void State_Trotting::calcCmd(){
    for(int i(0); i<4; ++i){
        if((*_contact)(i)){
            (*_q_contact).col(i)=_posFeetGlobal.col(i);
        }
    }
    calRotCToI();
    // std::cout << *_q_contact << std::endl;
    // std::cout << RotCToI << std::endl;
    /* Movement */
    _vCmdGlobal = RotCToI * _B2G_RotMat * _vCmdBody;
...
void State_Trotting::calcTau(){
    ...
    // _posError = _pcd - RotCToI.transpose()*_posBody;
    // _velError = _vCmdGlobal - RotCToI.transpose()*_velBody;
    pin_q.setZero();
    pin_q.segment(0,3)=_posBody;
    pin_q.segment(3,4)=_lowState->quaternion();
    pin_q.segment(7,12)=vec34ToVec12(_lowState->getQ());
    pin_u.setZero();
    pin_u.segment(0,3)=_velBody;
    pin_u[5]=_dYaw;
    pin_u.segment(6,12)=vec34ToVec12(_lowState->getQd());

    _ddPcd = _Kpp * _posError + _Kdp * _velError;  
    _dWbd  = _kpw*rotMatToExp(_Rd*_G2B_RotMat*(RotCToI.transpose())) + _Kdw * (RotCToI*_wCmdGlobal - _lowState->getGyroGlobal());
    ...
    // _forceFeetGlobal = - _balCtrl->calF(*_contact,pin_q,pin_u,_ddPcd,RotCToI);
    Vec34 accel;
    for(int i(0); i<4; ++i){
        if((*_contact)(i) == 0){
...
void State_Trotting::calRotCToI(){
   ...
   X.col(3)<<  1,
               ...
               1,
               1;
   X.block(0,0,4,3)=(*_q_contact).transpose();
